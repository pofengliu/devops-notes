...
- ## 團隊能了解自動化測試的內容
	...
	- 在不同的 pipeline 階段，自動化測試項目的
	  ...
	  * Smoke test stage: 在 commit stage 後，artifacts 應該會被佈署到 beta 或直接上到 staging 的環境。次時應該執行 smoke tests，這邊的案例應該專注於 end-to-end 的重要 CUJ (critical user journey) 及跟 3rd-party 的 integrated transactions.
- ## 關鍵實例 V.S. 一般測試案例
