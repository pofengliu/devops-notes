...
- ## 團隊能了解自動化測試的內容
	...
	- * Pipeline 的 build stage: 在 PR 階段，大部分的 AC 案例應該要可以在這階段跑完。AC 為已知的驗收條件，自動化應該利用大量的 mock, stub 和 testContainers 完成大部份 AC 的案例測試。盡量避免連結真正的 3rd-party API 或 database，因為在這階段只有你自己的代碼才能夠 fail 你的 pipeline。
	  * Smoke test stage: 在 commit stage 後，artifacts 應該會被佈署到 beta 或直接上到 staging 的環境。次時應該執行 smoke tests，這邊的案例應該專注於 end-to-end 的重要 CUJ (crit) 及跟 3rd-party 的 integrated transactions.
- ## 關鍵實例 V.S. 一般測試案例
