...
- ### Release Strategies
	...
	- 另外在網路層採用流量分流 (traffic splitting) 或是利用 Ingress 的 middleware 或是 service mesh 的方法來實作 blue-green deployment 或是 canary release，也不失為一個可考慮的方法。但是這邊要思考的點是，在微服務或架構複雜的情況下，這樣的作法能否達到同時有非常多的 features 在生產線同時做 canary 或 A/B testings 而彼此間不會互相影響
- ### Performance Metrics
